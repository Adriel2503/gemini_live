...
ni red: reciben diccionarios de stats como parametros y no dependen de
...
from gemini_live_demo.cli.runner import _finalize_turn, _new_turn_state, _note_vad_signal
from gemini_live_demo.core.events import summarize_event
from gemini_live_demo.core.metrics import MetricsCsv
...
def test_empty_turn_marker_is_ignored():
    ...
    kwargs = _kwargs(queue_stats={'max_size': 9})
    turn_index = _finalize_turn(**kwargs)
    assert turn_index == 0  # no incrementa
    assert kwargs['queue_stats']['max_size'] == 9  # tampoco resetea el backlog


def test_turn_with_text_increments_and_resets_queue_backlog():
    turn_state = _new_turn_state()
    turn_state['text_parts'] = ['hola']
    turn_state['turn_started_at'] = 9.0
    audio_queue: queue.Queue = queue.Queue()
    audio_queue.put(b'x')  # qsize() vale 1 al momento de cerrar el turno

    kwargs = _kwargs(turn_index=5, turn_state=turn_state, audio_queue=audio_queue, queue_stats={'max_size': 40})
    turn_index = _finalize_turn(**kwargs)
    ...
    assert kwargs['queue_stats']['max_size'] == 1
...
def test_first_audio_latency_none_when_no_audio_seen():
    turn_state = _new_turn_state()
    turn_state['text_parts'] = ['hola']
    turn_state['turn_started_at'] = 5.0
    turn_state['first_audio_at'] = None
    row_written = {}
...
def test_note_vad_signal_counts_start_and_end():
    ...
    assert vad_stats == {'start_count': 1, 'end_count': 1, 'last_type': 'END', 'last_offset': '20'}


def test_note_vad_signal_noop_without_activity():
    vad_stats = {'start_count': 0, 'end_count': 0, 'last_type': None, 'last_offset': None}
    _note_vad_signal(summarize_event({}), vad_stats)
    assert vad_stats == {'start_count': 0, 'end_count': 0, 'last_type': None, 'last_offset': None}
